...
void init_board()
{
    ...
    // Display is optional; ignore failure when board has no LCD
    ret = esp_board_device_init(ESP_BOARD_DEVICE_NAME_DISPLAY_LCD);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Failed to init display LCD device");
    }
}
